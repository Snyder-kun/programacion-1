...
export default class Cl_sPedido extends Cl_sProyecto {
  ...
  private static guardarPedidoLocal(pedido: any): { ok: boolean; mensaje: string } {
  ...
  static async agregar(pedido: Cl_mPedido): Promise<{ ok: boolean; mensaje: string }> {
    if (!pedido.nombre) return { ok: false, mensaje: "Nombre del cliente es obligatorio" };
    if (!pedido.tipoPago) return { ok: false, mensaje: "Forma de pago es obligatoria" };
    if (pedido.productos.length === 0) return { ok: false, mensaje: "Debe incluir al menos un producto" };
  ...
  static async getPedidos(): Promise<{ ok: boolean; tabla: Cl_mPedido[] }> {
    ...
    const solopedidos = datos.filter((d: any) => d.tipoPago && Array.isArray(d.productos));
  ...
  static async modificarEstado(id: number, procesado: boolean): Promise<{ ok: boolean; mensaje: string }> {
