export default class Cl_mCafetin {
    ...
    getFuenteIngresosPrincipal() {
        ...
        const codFuente = Object.keys(ingresosPorTipo).reduce((a, b) => ingresosPorTipo[a] > ingresosPorTipo[b] ? a : b);
    ...
    getStatsProductoPorId(idProducto) {
        let ventas = 0;
        let ingreso = 0;
        this._pedidos
            .filter((p) => p.procesado)
    ...
    getMontoTotalRecaudado() {
        const hoy = new Date();
        const inicioDia = new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate());
        const finDia = new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate() + 1);
        return this._pedidos
            .filter((p) => {
            if (!p.procesado || !p.fechaPago)
                return false;
