...
export default class Cl_cCliente {
    ...
    try {
      ...
      if (
        ...
        const desdeServidor = resultado.tabla
          .filter((p: any) => p && p.nombre && p.categoria)
          .map(
            (p: any) =>
              new Cl_mProducto({
                id: Number(p.id),
        ...
        if (desdeServidor.length > 0) {
          ...
          const nombresServidor = new Set(
            desdeServidor.map((p) => p.nombre.toLowerCase()),
          );
          const precargadosSinDuplicar = precargados.filter(
            (p) =>
              !idsServidor.has(p.id) &&
      ...
      this.vista.mostrarMensaje(
        "Error al conectar con el servidor. Usando catálogo local.",
        "warn",
      );
  ...
  private async enviarPedido(): Promise<void> {
    ...
    const productosPedido: Cl_mProducto[] = this.carrito.flatMap((i) => {
      const prod = this.modelo
        .getProductos()
