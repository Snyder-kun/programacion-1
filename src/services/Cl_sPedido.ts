...
export default class Cl_sPedido extends Cl_sProyecto {
  protected static override apiUrl =
    "https://6a5d1df80ad09982aef6e908.mockapi.io/cafetin_pedidos";
  private static localStorageKey = "cafetin_pedidos";

  private static obtenerPedidosLocal(): any[] {
    try {
  ...
  private static guardarPedidosLocal(pedidos: any[]): void {
    localStorage.setItem(this.localStorageKey, JSON.stringify(pedidos));
  }

  private static guardarPedidoLocal(pedido: any): {
    ok: boolean;
    mensaje: string;
  } {
    ...
    const nuevo = {
      ...pedido,
      id: maxId + 1,
      fechaHora: pedido.fechaHora ?? new Date().toISOString(),
    };
    pedidos.push(nuevo);
    this.guardarPedidosLocal(pedidos);
    return {
      ok: true,
      mensaje: `Pedido guardado localmente con ID: ${nuevo.id}`,
    };
  }
  ...
  static async agregar(
    ...
    if (
      pedido.tipoPago === "pago_movil" &&
      (!pedido.datosPagoMovil?.cedulaPago ||
        !pedido.datosPagoMovil?.bancoPago ||
        !pedido.datosPagoMovil?.referencia)
    ) {
  ...
  static async getPedidos(): Promise<{ ok: boolean; tabla: Cl_mPedido[] }> {
    ...
    const solopedidos = datos.filter(
      (d: any) => d.tipoPago && Array.isArray(d.productos),
    );
    const pedidos = solopedidos.map((d: any) => this.mapearPedido(d));
    return { ok: true, tabla: pedidos };
  }

  private static mapearPedido(d: any): Cl_mPedido {
    const productosMappeados = (d.productos || []).map((p: any) => {
  ...
  static async modificarEstado(
    ...
    const pedidoExistente = pedidosResponse.tabla.find(
      (p) => Number(p.id) === id,
    );

    if (!pedidoExistente) return { ok: false, mensaje: "Pedido no encontrado" };
    ...
    if (!existeRemoto.ok || !existeRemoto.existe) {
      ...
      if (indexLocal === -1) {
        return existeRemoto.ok
          ? { ok: false, mensaje: "Pedido no encontrado" }
          : { ok: false, mensaje: "Error de conexión" };
      }
      pedidosLocal[indexLocal] = {
        ...pedidosLocal[indexLocal],
        ...pedidoActualizado,
      };
