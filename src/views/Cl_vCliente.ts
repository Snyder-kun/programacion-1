...
export default class Cl_vCliente implements I_vCliente {
  ...
  constructor() {
    ...
    this.contenedorProductos = document.getElementById(
      "contenedorProductos",
    ) as HTMLElement;
    this.tbodyCarrito = document.getElementById(
      "tbodyCarrito",
    ...
    this.seccionPagoMovil = document.getElementById(
      "seccionPagoMovil",
    ) as HTMLElement;
    ...
    this.inReferencia = document.getElementById(
      "inReferencia",
    ) as HTMLInputElement;
    this.toastEl = document.getElementById("toast") as HTMLElement;

    this.selPago.onchange = () => {
      this.seccionPagoMovil.style.display =
        this.selPago.value === "pago_movil" ? "block" : "none";
    };
  }
  ...
  mostrarProductosCategoria(productos: Cl_mProducto[]): void {
    ...
    this.contenedorProductos.innerHTML = productos
      .map(
        (p) => `
      <div>
  ...
  actualizarCarrito(items: ItemCarrito[]): void {
    if (!items.length) {
      this.tbodyCarrito.innerHTML =
        '<tr><td colspan="4">Carrito vacío</td></tr>';
      return;
    }
    this.tbodyCarrito.innerHTML = items
      .map(
        (i) => `
      <tr>
        <td>${i.nombre}</td>
